...
#os.chdir(code_dir)
#from temp_norm_utils import norm_init
...
def save_results(respfile, Y, Yhat, S2, maskvol, Z=None, outputsuffix=None, 
...
def estimate(covfile, respfile, **kwargs):
    ...
    * estimating on a training dataset ouput of forward maps mean and se. 
    ...
    if saveoutput:
        if (run_cv or testresp is not None):
            save_results(respfile, Y[testids, :], Yhat[testids, :], S2[testids, :], maskvol, 
        ...
        if (run_cv or testresp is not None):
            output = (Y[testids, :], Yhat[testids, :], S2[testids, :], nm, Z[testids, :], 
                      results)
        else:
            output = (Y[testids, :], Yhat[testids, :], S2[testids, :], nm)
...
#        save_results(None, Yhat, S2, None, outputsuffix=outputsuffix)
        ...
        save_results(respfile, Y, Yhat, S2, maskvol, Z=Z, 
